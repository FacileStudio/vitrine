...
const isPrismaError = (cause: unknown): boolean =>
  !!cause &&
  typeof cause === 'object' &&
  'name' in cause &&
  typeof (cause as any).name === 'string' &&
  (cause as any).name.startsWith('PrismaClient');
...
const t = initTRPC.context<Context>().meta<OpenApiMeta>().create({
  errorFormatter({ shape, error, ctx, path }) {
    ...
    const isInternal =
      error.code === 'INTERNAL_SERVER_ERROR' || isPrismaError(error.cause);

    return {
      ...shape,
      message: isInternal ? 'An internal error occurred' : shape.message,
      data: {
        ...shape.data,
