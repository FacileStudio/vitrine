...
export default function ArchitectureModal({ open, setOpen }: ArchitectureModalProps) {
    return (
        <Dialog.Root open={open} onOpenChange={setOpen}>
            <Dialog.Portal>
                <Dialog.Overlay className="fixed inset-0 z-[120] bg-foreground/20 backdrop-blur-sm data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0" />

                <Dialog.Content className="fixed left-1/2 top-1/2 z-[121] flex max-h-[86vh] w-[92vw] max-w-[42rem] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-md bg-background text-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0">
                    <div className="flex items-start justify-between gap-6 border-b border-foreground/10 px-8 py-6">
                        <div>
                            <Dialog.Title className="text-[2.5rem] font-medium leading-tight text-foreground">
                                {architecture.title}
                            </Dialog.Title>
                            {/* <Dialog.Description className="mt-2 font-bb-mono text-[0.7rem] uppercase text-foreground/40">
                                {architecture.kicker}
                            </Dialog.Description> */}
                        </div>
