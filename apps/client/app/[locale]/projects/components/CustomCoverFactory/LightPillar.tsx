...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  const smoothMouseRef = useRef<THREE.Vector2>(new THREE.Vector2(0, 0));
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        if(uInteractive && length(uMouse) > 0.0) {
          float mouseAngle = uMouse.x * PI * 0.25;
  ...
  return (
    <div ref={containerRef} className={`w-full h-full absolute top-0 left-0 rounded-lg overflow-hidden ${className}`} style={{ mixBlendMode }} />
  );
};
